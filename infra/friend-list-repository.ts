...
export class FriendListRepositoryPostgres implements FriendListRepository {
    ...
    async findByUser(idUser: number): Promise<User[]> {
        return await database.manyOrNone('select distinct u.id, u.name from friend_list f, user_account u where (f.id_user = $1 or f.id_guest = $1)  and (f.id_user = u.id or f.id_guest = u.id)', [idUser]);
    }
